...
- Create an assignment directory called YOURCCID-MyMovieWishlist (e.g., hktang-MyMovieWishlist), and within it have one subdirectory (code), one text file (video.txt) and  a UML diagram file (UML.png).
- Your Android Studio project directory goes within the code subdirectory. The compiled binary APK file(s) should be found within an /app/build/intermediates/apk/debug/app-debug.apk subdirectory or /app/build/outputs/apk/debug/app-debug.apk within the project directory.
- You need to upload your (max three minute long) video to YouTube as an unlisted video. Then add the link of the video to the video.txt.
